...
//Read infutfile
	...
	ierr=fscanf(fp, "%d %d %d %f %f %f %f %f %f %f %f %f %f %f", &Nx,&Ny,&Nz,&Tmax,&plotgap,&Lx,&Ly,&Lz,&dt,&Du,&Dv,&A,&B,&errortol);
	...
	B=A+B;
//ALLocate the memory
	uhigh[0]=(float*) malloc(N*sizeof(float));
	vhigh[0]=(float*) malloc(N*sizeof(float));
	x=(float*) malloc(Nx*sizeof(float));
	y=(float*) malloc(Ny*sizeof(float));
	z=(float*) malloc(Nz*sizeof(float));
	time=(float*) malloc(1000000*sizeof(float));
	error=(float*) malloc(1000000*sizeof(float));
	tries=(int*) malloc(1000000*sizeof(int));
//allocat gpu mem	
	cl_uhigh[0] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_vhigh[0] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_uhigh[1] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_vhigh[1] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_ulow[0] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_vlow[0] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_ulow[1] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_vlow[1] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_uoldstep[0] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_voldstep[0] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_uoldstep[1] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
	cl_voldstep[1] = clCreateBuffer(context, CL_MEM_READ_WRITE, N * sizeof(float), NULL, &ret);
...
//get initial data
	...
	ret = clReleaseKernel(initialdata); ret = clReleaseProgram(p_initialdata);
        ret = clEnqueueReadBuffer(command_queue, cl_uhigh[0], CL_TRUE, 0, N * sizeof(float), uhigh[0], 0, NULL, NULL);
	ret = clFinish(command_queue);
        ret = clEnqueueReadBuffer(command_queue, cl_vhigh[0], CL_TRUE, 0, N * sizeof(float), vhigh[0], 0, NULL, NULL);
	ret = clFinish(command_queue);
...
//output of initial data U
	...
	for(i=0;i<N;i++){fwrite(&uhigh[0][i], sizeof(float), 1, fp);}
    	fclose( fp );	
//V
	...
	fp=fopen(nameconfig,"wb");
    	if (!fp) {fprintf(stderr, "Failed to write initialdata.\n"); exit(1); }
	for(i=0;i<N;i++){fwrite(&vhigh[0][i], sizeof(float), 1, fp);}
    	fclose( fp );
...
//initialise oldstep
	ret=clEnqueueCopyBuffer(command_queue,cl_uhigh[0],cl_uoldstep[0],0,0,N*sizeof(float),0,NULL,NULL);
	ret=clEnqueueCopyBuffer(command_queue,cl_vhigh[0],cl_voldstep[0],0,0,N*sizeof(float),0,NULL,NULL);
	ret=clEnqueueCopyBuffer(command_queue,cl_uhigh[1],cl_uoldstep[1],0,0,N*sizeof(float),0,NULL,NULL);
	ret=clEnqueueCopyBuffer(command_queue,cl_vhigh[1],cl_voldstep[1],0,0,N*sizeof(float),0,NULL,NULL);
...
//linear
	...
	ret = clFinish(command_queue);
        ret = clEnqueueReadBuffer(command_queue, cl_uhat[0], CL_TRUE, 0, N * sizeof(float), uhigh[0], 0, NULL, NULL);
        ret = clEnqueueReadBuffer(command_queue, cl_uhat[1], CL_TRUE, 0, N * sizeof(float), vhigh[0], 0, NULL, NULL);
	ret = clFinish(command_queue);
	...
	ret = clSetKernelArg(linearpart, 12, sizeof(float),(void* )&bhigh[0][0]);
	ret = clSetKernelArg(linearpart, 13, sizeof(float),(void* )&bhigh[0][1]);
...
//!a(1),b(1) done
	...
	ret=clEnqueueCopyBuffer(command_queue,cl_uhigh[0],cl_ulow[0],0,0,N*sizeof(float),0,NULL,NULL);
	ret=clEnqueueCopyBuffer(command_queue,cl_vhigh[0],cl_vlow[0],0,0,N*sizeof(float),0,NULL,NULL);
	ret=clEnqueueCopyBuffer(command_queue,cl_uhigh[1],cl_ulow[1],0,0,N*sizeof(float),0,NULL,NULL);
	ret=clEnqueueCopyBuffer(command_queue,cl_vhigh[1],cl_vlow[1],0,0,N*sizeof(float),0,NULL,NULL);
	ret = clFinish(command_queue);
...
//nonlinearpart
	...
	ret = clSetKernelArg(nonlinearpart, 5, sizeof(float),(void* )&ahigh[l][0]);
	ret = clSetKernelArg(nonlinearpart, 6, sizeof(float),(void* )&ahigh[l][1]);
...
//linear
	...
	ret = clSetKernelArg(linearpart, 10, sizeof(float),(void* )&A);
	ret = clSetKernelArg(linearpart, 11, sizeof(float),(void* )&B);
	ret = clSetKernelArg(linearpart, 12, sizeof(float),(void* )&bhigh[l][0]);
	ret = clSetKernelArg(linearpart, 13, sizeof(float),(void* )&bhigh[l][1]);
	ret = clSetKernelArg(linearpart, 14, sizeof(int),(void* )&Nx);
	ret = clSetKernelArg(linearpart, 15, sizeof(int),(void* )&Ny);
	ret = clSetKernelArg(linearpart, 16, sizeof(int),(void* )&Nz);
        ret = clEnqueueNDRangeKernel(command_queue, linearpart, 1, NULL, global_work_size, NULL, 0, NULL, NULL);
...
//nonlinearpart
	...
	ret = clSetKernelArg(nonlinearpart, 4, sizeof(float),(void* )&dt);
	ret = clSetKernelArg(nonlinearpart, 5, sizeof(float),(void* )&alow[l][0]);
	ret = clSetKernelArg(nonlinearpart, 6, sizeof(float),(void* )&alow[l][1]);
        ret = clEnqueueNDRangeKernel(command_queue, nonlinearpart, 1, NULL, global_work_size, NULL, 0, NULL, NULL);
	ret = clFinish(command_queue);
//linear
	...
	ret = clSetKernelArg(linearpart, 10, sizeof(float),(void* )&A);
	ret = clSetKernelArg(linearpart, 11, sizeof(float),(void* )&B);
	ret = clSetKernelArg(linearpart, 12, sizeof(float),(void* )&blow[l][0]);
	ret = clSetKernelArg(linearpart, 13, sizeof(float),(void* )&blow[l][1]);
	ret = clSetKernelArg(linearpart, 14, sizeof(int),(void* )&Nx);
	ret = clSetKernelArg(linearpart, 15, sizeof(int),(void* )&Ny);
	ret = clSetKernelArg(linearpart, 16, sizeof(int),(void* )&Nz);
        ret = clEnqueueNDRangeKernel(command_queue, linearpart, 1, NULL, global_work_size, NULL, 0, NULL, NULL);
...
//nonlinearpart a(4)
	...
	ret = clSetKernelArg(nonlinearpart, 3, sizeof(cl_mem),(void* )&cl_vlow[1]);
	ret = clSetKernelArg(nonlinearpart, 4, sizeof(float),(void* )&dt);
	ret = clSetKernelArg(nonlinearpart, 5, sizeof(float),(void* )&alow[3][0]);
	ret = clSetKernelArg(nonlinearpart, 6, sizeof(float),(void* )&alow[3][1]);
        ret = clEnqueueNDRangeKernel(command_queue, nonlinearpart, 1, NULL, global_work_size, NULL, 0, NULL, NULL);
	ret = clFinish(command_queue);
...
//uerror
	...
	ret = clFinish(command_queue);
        ret = clEnqueueReadBuffer(command_queue, cl_ulow[0], CL_TRUE, 0, N * sizeof(float), uhigh[0], 0, NULL, NULL);
	ret = clFinish(command_queue);
 	allerr[0]=0.0;
	int zahler=0;
	for(i=0;i<N;i++){
//printf("%f ",uhigh[0][i]);//-uhigh[0][i]);
...
//verror
	...
	ret = clEnqueueNDRangeKernel(command_queue, abserr, 1, NULL, global_work_size, NULL, 0, NULL, NULL);
	ret = clFinish(command_queue);
        ret = clEnqueueReadBuffer(command_queue, cl_vlow[0], CL_TRUE, 0, N * sizeof(float), uhigh[0], 0, NULL, NULL);
	ret = clFinish(command_queue);
 	allerr[1]=0.0;
 	zahler=0;
	for(i=0;i<N;i++){
//printf("%f ",uhigh[0][i]);//-uhigh[0][i]);
	...
	dt=fmin(plotgap,fmax(errortol,dt*pow((errortol*0.9/error[n]),(1.0/((float)p+1.0)))));
...
//if the error is too big we do the same timestep again, new stepsize
	if(error[n]>errortol){
		...
		ret=clEnqueueCopyBuffer(command_queue,cl_uoldstep[0],cl_uhigh[0],0,0,N*sizeof(float),0,NULL,NULL);
		ret=clEnqueueCopyBuffer(command_queue,cl_voldstep[0],cl_vhigh[0],0,0,N*sizeof(float),0,NULL,NULL);
		ret=clEnqueueCopyBuffer(command_queue,cl_uoldstep[1],cl_uhigh[1],0,0,N*sizeof(float),0,NULL,NULL);
		ret=clEnqueueCopyBuffer(command_queue,cl_voldstep[1],cl_vhigh[1],0,0,N*sizeof(float),0,NULL,NULL);
		ret = clFinish(command_queue);
	}
	else{
