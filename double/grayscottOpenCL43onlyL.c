...
//get grid first x
	...
	size_t global_work_size_x[3] = {Nx, 0, 0};
        size_t local_work_size_x[3]  = {Nx, 0, 0};
        ret = clEnqueueNDRangeKernel(command_queue, grid, 1, NULL, global_work_size_x, local_work_size_x, 0, NULL, NULL);
...
//then y
	...
	size_t global_work_size_y[3] = {Ny, 0, 0};
        size_t local_work_size_y[3]  = {Ny, 0, 0};
	ret = clEnqueueNDRangeKernel(command_queue, grid, 1, NULL, global_work_size_y, local_work_size_y, 0, NULL, NULL);
	ret = clFinish(command_queue);
...
//last z
	...
	size_t global_work_size_z[3] = {Nz, 0, 0};
        size_t local_work_size_z[3]  = {Nz, 0, 0};
	ret = clEnqueueNDRangeKernel(command_queue, grid, 1, NULL, global_work_size_z, local_work_size_z, 0, NULL, NULL);
	ret = clFinish(command_queue);
...
//get initial data
	...
	size_t global_work_size[3] = {N, 0, 0};
        size_t local_work_size[3]  = {Nx, 0, 0};
        ret = clEnqueueNDRangeKernel(command_queue, initialdata, 1, NULL, global_work_size, local_work_size, 0, NULL, NULL);
	ret = clFinish(command_queue);
...
//get frequencies first x
	...
	ret = clSetKernelArg(frequencies, 2, sizeof(int),(void*)&Nx);
        ret = clEnqueueNDRangeKernel(command_queue, frequencies, 1, NULL, global_work_size_x, local_work_size_x, 0, NULL, NULL);
	ret = clFinish(command_queue);
//then y
	...
	ret = clEnqueueNDRangeKernel(command_queue, frequencies, 1, NULL, global_work_size_y, local_work_size_y, 0, NULL, NULL);
	ret = clFinish(command_queue);
//last z
	...
	ret = clEnqueueNDRangeKernel(command_queue, frequencies, 1, NULL, global_work_size_z, local_work_size_z, 0, NULL, NULL);
	ret = clFinish(command_queue);
...
//linear
	...
	ret = clSetKernelArg(linearpart, 16, sizeof(int),(void* )&Nz);
        ret = clEnqueueNDRangeKernel(command_queue, linearpart, 1, NULL, global_work_size, local_work_size, 0, NULL, NULL);
	ret = clFinish(command_queue);
...
//nonlinearpart
	...
	ret = clSetKernelArg(nonlinearpart, 6, sizeof(double),(void* )&ahigh[l][1]);
        ret = clEnqueueNDRangeKernel(command_queue, nonlinearpart, 1, NULL, global_work_size, local_work_size, 0, NULL, NULL);
	ret = clFinish(command_queue);*/
//linear
	ret = clfftEnqueueTransform(planHandle, CLFFT_FORWARD, 1, &command_queue, 0, NULL, NULL,cl_uhigh, cl_uhat, tmpBufferu);
	ret = clfftEnqueueTransform(planHandle, CLFFT_FORWARD, 1, &command_queue, 0, NULL, NULL,cl_vhigh, cl_vhat, tmpBufferv);
...
//uerror
	...
	ret = clEnqueueNDRangeKernel(command_queue, abserr, 1, NULL, global_work_size, local_work_size, 0, NULL, NULL);
	ret = clFinish(command_queue);
