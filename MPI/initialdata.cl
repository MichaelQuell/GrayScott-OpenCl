...
#endif
__kernel void initialdata ( __global double2* u, __global double2* v, const int Nx, const int Ny,const int yoff,const double Lx,const double Ly)
{
   const int ind = get_global_id(0);

int i,j;
j=yoff+floor((double)(ind)/(double)Nx);
i=ind-(j-yoff)*Nx;
double x=(-1.0 + ( 2.0*(double)i/(double)Nx))*M_PI*Lx;
double y=(-1.0 + ( 2.0*(double)j/(double)Ny))*M_PI*Ly;
u[ind].x=0.8-exp(-1.0*(x*x+y*y )-1.0);	
